...
async function initializeKsqlDB() {
  try {
    const response = await axios.post(`${API_URL}/api/ksqldb/initialize`);
 ...
 */
async function produceMessage(message: string, key?: string) {
  try {
    const response = await axios.post(`${API_URL}/api/kafka/produce`, {
      message,
      key
    });
 ...
 */
async function getLatestMessageById(id: string) {
  try {
    const response = await axios.get(`${API_URL}/api/ksqldb/messages/${id}`);
...
async function executeQuery(query: string) {
  try {
    const response = await axios.post(`${API_URL}/api/ksqldb/query`, { query });
